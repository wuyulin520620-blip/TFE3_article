...
immune.combined = subset(scRNA,Celltype %in% c('B','Plasma'))
...
testAB.integrated <- FindNeighbors(scRNA_harmony, reduction = "harmony", dims = 1:30) %>% FindClusters(resolution =  0.1)
...
celltype = data.frame(
  cluster = c(0,1,2,4),
  celltype = c('B_naive','B_activated','Plasma',
               'Switched_memony_B'
  )
)
celltype 
